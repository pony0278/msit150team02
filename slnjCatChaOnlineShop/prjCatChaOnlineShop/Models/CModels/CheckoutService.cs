    ...
    public class CheckoutService
    {
        ...
        public CGetCouponPrice GetLoyaltyPoints()
        {
           CGetCouponPrice getCouponPrice= new CGetCouponPrice();
            ...
            ////會員ID
            int memId= (int)GetCurrentMemberId();
            ...
            if (memId != null && cart != null)
            {
                //會員現有的紅利點數
                var loyaltyPoints = _context.ShopMemberInfo.Where(m => m.MemberId == memId).Select(m => m.LoyaltyPoints).FirstOrDefault();
                //計算現有的紅利等於多少元
                var transferPoint = loyaltyPoints / 100;
                //會員初始的購物車金額
                ...
                {
                    getCouponPrice.firstloyaltyPoints = (int)loyaltyPoints;
